...
void EthernetTransmit(unsigned char *buffer, int length)
{
   ...
   //cnt++;
   //trft=1;
   //puts("MemoryWrite(ETHERNET_REG, length)\n");
   //Led(0);
...
{  
   ...
   for(;;)
   {
      ...
      for(;;)
      {
         ...
         if(length == 0)
            break;
         puts("led1 --> EthernetReceive !=0\n");
		 //Led(1);
         rc = IPProcessEthernetPacket(ethFrame, length);
		 puts("led0 --> IPProcessEthernetPacket \n");
		 //Led(0);
         if(rc)
            ethFrame = NULL;
      }
...
{	
   ...
   EtherThread=OS_ThreadCreate("eth", EthernetThread,/*(uint32*)k*/NULL ,240, 0);
   puts("--> Ethernet_Thread\n\n");
   printf("EtherThread=0x%x info=0x%x priority=%d\n", 
     EtherThread, OS_ThreadInfoGet(EtherThread, 0),OS_ThreadPriorityGet(EtherThread));
