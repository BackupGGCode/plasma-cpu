...
lect_ent: 
	$(AS_MIPS) -o boot.o boot.asm
	$(GCC_MIPS) no_os.c 
	$(GCC_MIPS) lecture_entrees.c
	$(LD_MIPS) -Ttext 0 -eentry -Map test.map -s -N -o test.axf \
		boot.o lecture_entrees.o no_os.o
	-$(DUMP_MIPS) --disassemble test.axf > test.lst
	$(CONVERT_BIN)
	$(CP) code.txt $(VHDL_DIR)
...
chenillard_switch: 
	$(AS_MIPS) -o boot.o boot.asm
	$(GCC_MIPS) chenillard_switch.c
	$(GCC_MIPS) no_os.c 
	$(LD_MIPS) -Ttext 0 -eentry -Map test.map -s -N -o test.axf \
		boot.o chenillard_switch.o no_os.o 
	-$(DUMP_MIPS) --disassemble test.axf > test.lst
	$(CONVERT_BIN)
	$(CP) code.txt $(VHDL_DIR)
